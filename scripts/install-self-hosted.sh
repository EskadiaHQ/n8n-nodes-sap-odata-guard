...
EXPECTED_SHA256=458ab4a3cf68721aadb2a45775b67fda0ac0766294509d0cb09fcbfd463bf3ca
...
if [ -e "$target" ]; then
  ...
  if [ "$installed" = "$PACKAGE_VERSION" ]; then
    ...
    require(\"$target/dist/nodes/SapOdataGuard/SapOdataGuard.node.js\");
    require(\"$target/dist/credentials/SapOdataGuardApi.credentials.js\");
    require(\"$target/dist/credentials/SapOdataGuardOAuth2Api.credentials.js\");
    "
...
cp "$manifest" "$manifest.pre-sap-odata-guard-$stamp"
